...
const toAbsoluteUrl = (url: string): string => {
  ...
  return `https://www.nytimes.com/${url.replace(/^\//, '')}`
}
...
const extractImageUrl = (
  ...
  if (isMultimediaArray(multimedia)) {
    const mainItem = multimedia.find((m) => m.subtype !== 'thumbnail')
    const item = mainItem ?? multimedia.find((m) => m.subtype === 'thumbnail')
    return item ? toAbsoluteUrl(item.url) : null
  }

  if (multimedia.default?.url) {
    return toAbsoluteUrl(multimedia.default.url)
  }

  if (multimedia.thumbnail?.url) {
    return toAbsoluteUrl(multimedia.thumbnail.url)
  }
...
export class NyTimesAdapter implements IArticleAdapter {
  ...
  async fetch(
    ...
    const articles: Article[] = Array.isArray(data.docs)
      ? data.docs.map((item) => ({
          id: `nytimes-${item._id}`,
